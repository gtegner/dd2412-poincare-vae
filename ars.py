...
# ARS implementation in pytorch
# Taken from authors code with in turn is inspired by https://github.com/alumbreras/ARS/blob/master/ars.py
...
class ARS():
    ...
    def __init__(self, logpdf, grad_logpdf, xi, lb=-infty, ub=infty, use_lower=False, ns=50, **fargs):
        '''
        ...
        if not (self.hprime[:, self.K-1] < 0).all():
            raise IOError(
                'initial anchor points must span mode of PDF (right)')
        self.insert()
    ...
    def sample(self, shape=torch.Size()):
        ...
        while bool_mask.sum() != 0:
            ...
            ht = ht - self.offset
            ut = self.h.gather(1, i) + (xt - self.x.gather(1, i)
                                        ) * self.hprime.gather(1, i)
    ...
    def insert(self, nbnew=0, xnew=None, hnew=None, hprimenew=None):
        ...
        self.z[:, 1:self.K] = (diff(self.h[:, :self.K]) - diff(
            self.x[:, :self.K] * self.hprime[:, :self.K])) / -diff(self.hprime[:, :self.K])
        ...
        self.s[self.hprime[:, :self.K] ==
               0.] = 0.  # should be 0 when gradient is 0
        self.cs = torch.cat((torch.zeros(self.B, 1).to(
            self.device), torch.cumsum(self.s, dim=-1)), dim=-1)
        self.cu = self.cs[:, -1]
    ...
    def sampleUpper(self, shape=torch.Size()):
        ...
        i = (self.cs/self.cu.unsqueeze(-1)
             ).unsqueeze(-1) <= u.unsqueeze(1).expand(*self.cs.shape, *shape)
        idx = i.sum(1) - 1
